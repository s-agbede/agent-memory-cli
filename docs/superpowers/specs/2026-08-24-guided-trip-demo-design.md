...
For every non-empty onboarding answer, the CLI first asks the LLM for a brief, fact-preserving normalized value, then wraps it in category-specific wording and writes an owner-scoped semantic long-term-memory record directly. In particular, the origin answer is stored as the traveler's usual departure city rather than inferred residence. It displays that the profile fact is ready to recall immediately. It batches the four records where possible and reports per-record failure without exposing credentials. These explicit profile facts solve cold start.

Normal free-form chat retains the existing required order: the user's detail is first stored as a session event, session context is loaded, the direct owner profile is loaded with a filter-only request, relevant long-term context is retrieved semantically, an answer is generated, and the assistant response is stored as a session event. Direct profile records are placed before semantic results and repeated Redis record IDs are removed. The CLI prints that the session event is saved and Redis Agent Memory evaluates it for asynchronous promotion. Redis Agent Memory handles extraction, consolidation, and service-side deduplication; the CLI does not poll or implement a competing conflict-resolution rule.
